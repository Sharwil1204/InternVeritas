...
const getMockCompanyResults = (companyName: string): CompanyOption[] => {
  return [
    {
      id: '1',
      name: companyName,
    ...
    {
      id: '2',
      name: `${companyName} Technologies`,
      website: `https://${companyName.toLowerCase().replace(/\s+/g, '')}tech.com`,
      description: 'Software development and IT solutions provider',
    },
    {
      id: '3',
      name: `${companyName} Solutions`,
      website: `https://${companyName.toLowerCase().replace(/\s+/g, '')}solutions.io`,
      description: 'Business process and digital transformation services',
    },
    {
      id: '4',
      name: `${companyName} Inc.`,
      website: `https://${companyName.toLowerCase().replace(/\s+/g, '')}inc.com`,
      description: 'Enterprise software and cloud computing platform',
    },
  ];
};

// Mock website verification
const mockVerifyWebsite = (company: CompanyOption): CompanyVerificationData => {
  // Randomize results for realistic feel
  const isLive = Math.random() > 0.2; // 80% chance website is live
  const hasSsl = Math.random() > 0.15; // 85% chance has SSL
  const postingStatuses: ('active' | 'ended' | 'not-found')[] = ['active', 'ended', 'not-found'];
  const randomStatus = postingStatuses[Math.floor(Math.random() * postingStatuses.length)];
  
  return {
    companyName: company.name,
    domainAge: `${Math.floor(Math.random() * 15) + 1} years`,
    sslCertificate: hasSsl,
    websiteLive: isLive,
    internshipPostingStatus: isLive ? randomStatus : 'not-found',
    skipped: false,
  };
};
...
export const CompanySelectionModal = ({
  ...
  const companies = getMockCompanyResults(companyName);
  ...
  const handleCompanySelect = async (company: CompanyOption) => {
    ...
    setTimeout(() => {
      const verification = mockVerifyWebsite(company);
  ...
  const handleCustomUrlSubmit = async () => {
    ...
    // Mock verification with custom URL
    setTimeout(() => {
      const verification: CompanyVerificationData = {
        companyName: companyName,
        domainAge: `${Math.floor(Math.random() * 12) + 1} years`,
        sslCertificate: customUrl.startsWith('https://'),
        websiteLive: true,
        internshipPostingStatus: Math.random() > 0.5 ? 'active' : 'not-found',
        skipped: false,
      };
      setIsVerifying(false);
      onSelect(verification);
        ...
        {isVerifying ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/60 text-sm">
              Checking website status, domain info, and career postings
            </p>
          </div>
        ) : (
          <>
            ...
            <p className="text-white/70 mb-6 text-sm">
              We found multiple companies matching "{companyName}". Please select the correct one:
            </p>
