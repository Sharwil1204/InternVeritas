...
export const AnalyzerPage = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setFormData({ ...formData, advertisementFile: file });
      // Read file content if it's text
      if (file.type === 'text/plain') {
        const reader = new FileReader();
        reader.onload = (event) => {
          setFormData((prev) => ({ ...prev, advertisement: event.target?.result as string }));
        };
  ...
  const detectCompanyInAd = (text: string) => {
    // Simple mock detection - in real app would use NLP
    const companies = ['TechCorp', 'GlobalSolutions', 'InnovateLab'];
  ...
  const handleNextStep = () => {
    if (currentStep === 1) {
      // Check if company name is entered and show company selection modal
      ...
      if (formData.companyName && formData.advertisement) {
        const detected = detectCompanyInAd(formData.advertisement);
        if (detected && detected.toLowerCase() !== formData.companyName.toLowerCase()) {
  ...
  const handleAnalyze = () => {
    ...
    const existingAnalyses = JSON.parse(localStorage.getItem('internveritas_analyses') || '[]');
    existingAnalyses.push(analysis);
    localStorage.setItem('internveritas_analyses', JSON.stringify(existingAnalyses));
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <button
                    ...
                    disabled={!canProceed()}
                    className="w-full px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Next
                  </button>
                </div>
                    ...
                    {['yes', 'no', 'not-sure'].map((option) => (
                      <button
                        ...
                        className={`p-6 rounded-xl border-2 transition-all ${
                          formData.paymentRequired === option
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {['yes', 'no', 'not-sure'].map((option) => (
                      <button
                        ...
                        className={`p-6 rounded-xl border-2 transition-all ${
                          formData.interviewProcess === option
                            ? 'border-violet-500 bg-violet-500/10'
                            : 'border-white/10 hover:border-white/20'
                        }`}
                      >
