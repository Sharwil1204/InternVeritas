...
export const LandingPage = () => {
  ...
  const features = [
    ...
    { icon: DollarSign, title: 'Payment Detection', description: 'Flags requests for money or financial information' },
    { icon: Building2, title: 'Company Verification', description: 'Cross-references company information with online presence' },
    { icon: MessageSquare, title: 'Language Analysis', description: 'Detects unprofessional or manipulative communication patterns' },
    { icon: Video, title: 'Interview Process Check', description: 'Evaluates legitimacy of interview procedures and requests' },
  ];
        ...
        <section className="relative min-h-[90vh] flex items-center">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                {[
                  ...
                  { icon: CheckCircle2, text: '5 Factor Verification' },
                  { icon: Zap, text: 'Real-time Results' },
                ].map((badge, idx) => (
                  <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 + idx * 0.2 }} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="relative">
              <div className="p-8 rounded-2xl border border-white/20 relative overflow-hidden" style={{ background: 'rgba(255, 255, 255, 0.12)', backdropFilter: 'blur(20px)' }}>
                ...
                <div className="relative">
                  ...
                  <div className="flex justify-center mb-6">
                    <div className="relative w-40 h-40">
                      <svg className="transform -rotate-90 w-40 h-40">
                        ...
                        <circle cx="80" cy="80" r="70" stroke="#f97316" strokeWidth="12" fill="none" strokeDasharray={`${(78 / 100) * 440} 440`} strokeLinecap="round" />
                      </svg>
                  ...
                  <div className="space-y-3">
                    {[{ label: 'Advertisement', value: 65 }, { label: 'Email', value: 85 }, { label: 'Company', value: 90 }].map((factor, idx) => (
                      <div key={idx}>
                        ...
                        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                          <motion.div
                            ...
                            className={`h-full rounded-full ${factor.value >= 80 ? 'bg-red-500' : factor.value >= 60 ? 'bg-orange-500' : 'bg-green-500'}`}
                          />
                        </div>
                      </div>
                    ))}
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {features.map((feature, idx) => (
                  ...
                  >
                    ...
                    <h3 className="text-lg mb-2 text-white group-hover:text-violet-200 transition-colors">{feature.title}</h3>
                    <p className="text-white/60 group-hover:text-white/80 transition-colors text-sm">{feature.description}</p>
                  </motion.div>
                ))}
              </div>
            </div>
                ...
                {steps.map((step, idx) => (
                  <div key={idx} className="flex items-center">
                    ...
                    >
                      ...
                      <p className="text-sm text-white/80 group-hover:text-white transition-colors">{step.title}</p>
                    </motion.div>
                    {idx < steps.length - 1 && <ArrowRight className="h-6 w-6 text-violet-400 mx-2 hidden md:block" />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <section id="about" className="py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-16">
                ...
                <h2 className="text-4xl md:text-5xl">Protecting the Next Generation of Talent</h2>
              </div>
              ...
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="space-y-6">
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-600/20 border border-violet-500/30">
                    ...
                    <span className="text-violet-300 text-sm">Smart Scam Detection</span>
                  </div>

                  <p className="text-white/70 text-base leading-relaxed">
                    InternVeritas uses AI-powered analysis combined with rule-based detection to evaluate internship offers across multiple factors — helping students make informed decisions before applying.
                  </p>

                  <p className="text-white/70 text-base leading-relaxed">
                    Internship scams can lead to financial loss, identity theft, and wasted time. Our platform analyzes 6 key factors so you can pursue genuine opportunities with confidence.
                  </p>

                  <div className="flex flex-col gap-3 pt-2">
                    {[
                      { icon: Shield, text: 'No similar platform exists anywhere' },
                      { icon: Zap, text: 'Results in seconds — free to use' },
                      { icon: Brain, text: 'Dual-layer AI + rule-based detection' },
                    ].map((point, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-violet-600/20 border border-violet-500/30 flex items-center justify-center flex-shrink-0">
                          <point.icon className="h-4 w-4 text-violet-400" />
                        </div>
                ...
                <div className="grid gap-4">
                  {[
                    ...
                    { number: 1, suffix: '', label: 'AI-Powered', desc: 'Intelligent pattern recognition' },
